...
BEGIN
	...
	BEGIN
		...
		DECLARE
			...
			@ICMSSTModalidade SMALLINT = 0,
			@ICMSSTCalcula BIT = 0,
			@ICMSSTPercentual DECIMAL(7, 2) = 0.00,
			@ICMSSTBase MONEY = 0.00,
			@ICMSSTReducao DECIMAL(7, 2) = 0.00,
			@ICMSSTValor MONEY = 0.00,
			@ICMSSTMVA DECIMAL(7, 2) = 0.00,
			@IPICalcula	BIT = 0,
		...
		SELECT
			...
			@ICMSCSTA = ICMS.CSTA, @ICMSCSTB = ICMS.CSTB, @ICMSCSOSN = ICMS.CSOSN,
			@ICMSCalcula = Ope.CalculaICMS, @ICMSPercentual = ICMS.ICMSPercentual, @ICMSReducao = ICMS.ICMSReducao, @ICMSTributacao = ICMS.ICMSTributacao,
			@OperacaoCalculaICMSST = Ope.CalculaICMSSubst,
			@ICMSSTCalcula = Ope.CalculaICMSSubst, @ICMSSTPercentual = ICMS.ICMSSubsPercentual, @ICMSSTReducao = ICMS.ICMSSubsReducao,
			@ICMSSTModalidade = ICMS.ModalidadeBaseICMSSubs, @ICMSSTMVA = ICMS.ICMSSubsMVA
		FROM
			Operacoes Ope
		...
		BEGIN
			...
			SET @ICMSValor = 0.00;
		END;
		IF (@OperacaoCalculaICMSST <> 1)
			SET @ICMSSTCalcula = 0;
		IF ((@ICMSSTCalcula = 1) AND (@ICMSSTPercentual > 0))
		BEGIN
			SET @ICMSSTBase = @PrecoTotal;
			IF (@ICMSSTModalidade = 4)
				SET @ICMSSTBase = @PrecoTotal + @IPIValor;
			SET @ICMSSTBase = @ICMSSTBase + ROUND(@ICMSSTBase * (@ICMSSTMVA / 100.0), 2, 1);
			IF (@ICMSSTReducao > 0)
				SET @ICMSSTBase = ROUND((@PrecoTotal - @PedidoProdutoDesconto) * (1 - (@ICMSSTReducao / 100.0)), 2, 1);
			SET @ICMSSTValor = ROUND(@ICMSSTBase * (@ICMSSTPercentual / 100.0), 2, 1) - @ICMSValor;
		END ELSE
		BEGIN
			SET @ICMSSTPercentual = 0.00;
			SET @ICMSSTBase = 0.00;
			SET @ICMSSTReducao = 0.00;
			SET @ICMSSTValor = 0.00;
		END;
		-- Atualiza
		UPDATE vwPedidosProdutos SET
			...
			ICMSValor = @ICMSValor,
			ICMSSubsBase = @ICMSSTBase,
			ICMSSTPercRedBaseCalc = @ICMSSTReducao,
			ICMSSubsPercentual = @ICMSSTPercentual,
			ICMSSubsValor = @ICMSSTValor,
